...
\setlength{\parindent}{0pt} % no indent automatically everywhere
...
\maketitle
In this document we define small-step rules for the datatype in \texttt{math-expr.lean}. We then show that the \texttt{eval} function defined in Lean corresponds to these small-step rules. %The proofs are paper proofs, but also implemented in Lean.
...
We write $\Gamma\vdash e \to^{*} e'$ for the reflexive-transitive closure of $\to$. Formally, $\Gamma \vdash e \to^{*}e'$ iff either $e=e'$ (reflexive) or there exists $e''$ with $\Gamma \vdash e \to^{*} e''$ and $\Gamma\vdash e''\to e'$ (transitive).
...
\item[Div.] Let $s_1 : \Gamma\vdash e_1\to^{*}\operatorname{const}(r_1)$ and $s_2 : \Gamma\vdash e_2\to^{*}\operatorname{const}(r_2)$ with $r_2\neq 0$. Using context lifting with $C_L[t]=\operatorname{div}(t,e_2)$ (E-DivL) we obtain
\[ L : \Gamma\vdash \operatorname{div}(e_1,e_2)\to^{*}\operatorname{div}(\operatorname{const}(r_1),e_2). \]
Then lifting $s_2$ with $C_R[t]=\operatorname{div}(\operatorname{const}(r_1),t)$ (E-DivR) yields
...
Finally apply E-Div (using $r_2\neq 0$) to extend $R$, and concatenate with $L$ to derive $\Gamma\vdash \operatorname{div}(e_1,e_2)\to^{*}\operatorname{const}(r_1/r_2)$. 
\emph{Remark:} Our small-step semantics enforces a left-to-right discipline: we may only reduce the right operand of a division once the left has become a constant (rule E-DivR requires a constant on the left). In contrast, the Lean \texttt{eval} function inspects the denominator first to short-circuit on division by zero. These orders therefore differ operationally; a right-first reduction sequence is \emph{not} derivable from the stated small-step rules. This does not affect extensional equivalence: whenever both operands evaluate successfully to nonzero constants, there exists a left-first small-step derivation to the same result; and if evaluation returns none (due to a stuck subexpression or zero denominator), the left-first semantics still reaches a stuck term. We rely only on existence of some derivation, not mirroring \texttt{eval}'s order.
\end{description}
\end{proof}
...
We say an expression $e$ is \emph{stuck} iff it is not a value (not of the form $\operatorname{const}(q)$ for any $q\in\mathbb{Q}$) and it cannot take a step: there is no $e'$ with $\Gamma\vdash e\to e'$. This includes both terminal stuck forms and compound expressions that contain them (e.g., $\operatorname{add}(\operatorname{var}(x),e_2)$ where $\Gamma(x)=\bot$).
...
\item[Var.] We have $e = \operatorname{var}(x)$ for some string $x$. Then $\operatorname{eval}(\operatorname{var}(x),\Gamma)=\text{some }q$. By definition of eval we have $\Gamma[x]?=\text{some }q$ and thus $\Gamma(x)=q$. Applying E-Var once yields $\Gamma\vdash \operatorname{var}(x)\to^{*}\operatorname{const}(q)$.
...
\end{align*}
We apply our induction hypothesis to both evaluations to obtain $\Gamma\vdash e_1\to^{*}\operatorname{const}(r_1)$ and $\Gamma\vdash e_2\to^{*}\operatorname{const}(r_2)$; the division reduction lemma (using $r_2\neq 0$) yields $\Gamma\vdash\operatorname{div}(e_1, e_2)\to^{*}\operatorname{const}(r_1/r_2)$. \emph{Remark:} Here we construct a left-first small-step derivation even though \texttt{eval} inspects $e_2$ first. Matching the evaluation order is unnecessary; the rules guarantee a deterministic left-first path whenever both sides succeed, and any failure detected early by \texttt{eval} corresponds to a stuck term reachable under left-first reduction.
\end{description}
\end{proof}
...
\emph{Note:} If $e_1$ is stuck already (non-constant with no step), evaluation would also yield none after inspecting $e_2$ (whether $e_2$ produces none or a nonzero value); the multi-step argument treats all branches uniformly by reducing each operand (when possible) before forming a stuck division term.
\end{description}
...
\begin{description}
    ...
    \item[Add. ] We have $n=\operatorname{add}(n_1,n_2)$ stuck. This occurs (under left-to-right rules) in exactly two mutually exclusive ways:
    \begin{enumerate}[label=(\alph*)]
      \item $n_1$ is stuck (non-constant). Then by IH $\operatorname{eval}(n_1,\Gamma)=\text{none}$ and
        \[
        \operatorname{eval}(\operatorname{add}(n_1,n_2),\Gamma)=\text{do }(\leftarrow \text{none}) + (\leftarrow \operatorname{eval}(n_2,\Gamma))=\text{none}.
        \]
      \item $n_1=\operatorname{const}(r_1)$ and $n_2$ is stuck (non-constant). Then $\operatorname{eval}(n_1,\Gamma)=\text{some }r_1$ while by IH $\operatorname{eval}(n_2,\Gamma)=\text{none}$, so
        \[
        \operatorname{eval}(\operatorname{add}(\operatorname{const}(r_1),n_2),\Gamma)
        =\text{do }(\leftarrow \text{some }r_1) + (\leftarrow \text{none})=\text{none}.
        \]
    \end{enumerate}
    In either subcase the evaluation yields none.
    \item[Sub. ] $n=\operatorname{sub}(n_1,n_2)$ stuck. Exactly analogous dichotomy:
    \begin{enumerate}[label=(\alph*)]
      \item $n_1$ stuck non-constant $\Rightarrow \operatorname{eval}(n_1,\Gamma)=\text{none}$ and the subtraction monad short-circuits to none.
      \item $n_1=\operatorname{const}(r_1)$ and $n_2$ stuck non-constant $\Rightarrow \operatorname{eval}(n_2,\Gamma)=\text{none}$ and the whole evaluates to none.
    \end{enumerate}
    \item[Mul. ] $n=\operatorname{mul}(n_1,n_2)$ stuck. Same two cases:
    \begin{enumerate}[label=(\alph*)]
      \item $n_1$ stuck non-constant $\Rightarrow$ left evaluation none, product none.
      \item $n_1=\operatorname{const}(r_1)$ and $n_2$ stuck non-constant $\Rightarrow$ right evaluation none, product none.
    \end{enumerate}   

    \item[Div. ] We have $n=\operatorname{div}(n_1,n_2)$ stuck. For $n$ to be stuck we need E-DivL, E-DivR, and E-Div all to fail. E-DivL fails means $n_1$ cannot step, so either $n_1$ is stuck (and not a constant) or $n_1=\operatorname{const}(r_1)$ for some $r_1$.
...
Thus every stuck (non-value) expression evaluates to none.
\end{proof}

\begin{theorem}[Equivalence Summary]
For all environments $\Gamma$ and expressions $e$:
By Theorem 1 and 2: $\operatorname{eval}(e,\Gamma)=\text{some }q \iff \Gamma\vdash e\to^{*}\operatorname{const}(q)$
and by Theorem 3: $\operatorname{eval}(e,\Gamma)=\text{none} \iff \exists n.\ \Gamma\vdash e\to^{*} n \text{ and } n \text{ is stuck}$.
\end{theorem}
